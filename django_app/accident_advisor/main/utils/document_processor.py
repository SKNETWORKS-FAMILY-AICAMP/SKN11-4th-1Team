...
class DocumentProcessor:
    ...
    def convert_term_to_docs(cls, data_list: List[Dict]) -> List[Document]:
        ...
        return [
            Document(
                page_content=f"{item[cls.METADATA_KEY['TERM']['TERM']]} : {item[cls.METADATA_KEY['TERM']['DESC']]}",
                metadata={
                    cls.METADATA_KEY['TERM']['TERM']: item[cls.METADATA_KEY['TERM']['TERM']]
                }
    ...
    def convert_precedent_to_docs(cls, data_list: List[Dict]) -> List[Document]:
        ...
        return [
            Document(
                page_content=f"{item[cls.METADATA_KEY['PRECEDENT']['COURT']]} {item[cls.METADATA_KEY['PRECEDENT']['CASE_ID']]} : {item[cls.METADATA_KEY['PRECEDENT']['CONTENT']]}",
                metadata={
                    cls.METADATA_KEY['PRECEDENT']['COURT']: item[cls.METADATA_KEY['PRECEDENT']['COURT']],
                    cls.METADATA_KEY['PRECEDENT']['CASE_ID']: item[cls.METADATA_KEY['PRECEDENT']['CASE_ID']],
                }
            ) for item in data_list
        ]
    ...
    def convert_car_case_to_docs(cls, data_list: List[Dict]) -> List[Document]:
        ...
        for item in data_list:
            ...
            reason = item.get(cls.METADATA_KEY['ACCIDENT_CASE']['REASON'])
            if isinstance(reason, list):
                reason = "\\n".join(map(str, reason))

            metadata = {
                "type": "car_case",
                "id": safe_value(item.get(cls.METADATA_KEY['ACCIDENT_CASE']['CASE_ID'])),
                "title": safe_value(item.get(cls.METADATA_KEY['ACCIDENT_CASE']['CASE_TITLE'])),
                "situation": safe_value(item.get(cls.METADATA_KEY['ACCIDENT_CASE']['CASE_SITUATION'])),
                "base_ratio": safe_value(item.get(cls.METADATA_KEY['ACCIDENT_CASE']['BASE_RATIO'])),
                "modifiers": safe_value(item.get(cls.METADATA_KEY['ACCIDENT_CASE']['MODIFIERS'])),
                "load_traffic_law": safe_value(item.get(cls.METADATA_KEY['ACCIDENT_CASE']['LAW_REFERENCES'])),
                "precedent": safe_value(item.get(cls.METADATA_KEY['ACCIDENT_CASE']['PRECEDENT'])),
    ...
    def load_and_convert_all_documents(cls, metadata_path: Path) -> Dict[str, List[Document]]:
        ...
        """
        # 파일 경로 정의 (traffic_law_rag.json만 사용)
        file_paths = {
            ...
            'modifier': metadata_path / 'modifier.json',
        }
        ...
        try:
            ...
            car_case_docs = cls.convert_car_case_to_docs(car_case_data)
            
            # modifier 문서 (car_case와 합침)
            if file_paths['modifier'].exists():
                modifier_data = cls.load_json(str(file_paths['modifier']))
                modifier_docs = cls.convert_list_to_documents(modifier_data, 'modifier')
                documents['car_case'] = car_case_docs + modifier_docs
            else:
